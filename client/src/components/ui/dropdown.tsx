  ...
  return (
    <div ref={dropdownRef} className={`relative ${className}`}>
      ...
      >
        ...
        <ChevronDown
          className={`w-4 h-4 text-[#505050] transition-transform duration-200 ${isOpen ? "rotate-180" : ""}`}
        />
      </button>
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="absolute left-0 right-0 top-full mt-1 py-1 rounded-lg z-50 bg-[#1A1A1A] border border-[#252525] shadow-xl max-h-60 overflow-y-auto"
          >
            {options.map((option) => (
              <motion.button
                key={option.value}
